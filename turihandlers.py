...
class UpdateModelForDatasetId2(BaseHandler):
    def get(self):
        '''Train a new model (or update) for given dataset ID
        '''
        dsid = self.get_int_arg("dsid",default=0)
        ...
        if len(data)>0:
            ...
            model = tc.svm_classifier.create(data,target='target',verbose=0)# training
            yhat = model.predict(data)
            self.clf[dsid] = model
            acc = sum(yhat==data['target'])/float(len(data))
            # save model for use later, if desired
    ...
    def get_features_and_labels_as_SFrame(self, dsid):
        ...
        # print(len(decodedFeatures)) #uncomment this to check the amount of uploaded image in this dsid
        data = {
            'target': labels,
            'sequence': np.array(decodedFeatures)
        }
